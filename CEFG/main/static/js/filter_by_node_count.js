document.addEventListener('DOMContentLoaded', () => {
    ...
    select.addEventListener('change', async (event) => {
        ...
        if (!selectedCount) {
            resultsContainer.innerHTML = '<p class="empty-state">Please select a node count.</p>';
            return;
        }
        
        try {
            const response = await fetch(`/graphs/node-count/${encodeURIComponent(selectedCount)}/`);
    ...
    function displayResults(data) {
        if (!data.graphs || data.graphs.length === 0) {
            resultsContainer.innerHTML = `<p class="empty-state">No graphs found with ${data.node_count} nodes.</p>`;
            return;
        }
        
        const header = document.createElement('div');
        header.className = 'results-header';
        header.textContent = `Found ${data.count} graph${data.count !== 1 ? 's' : ''} with ${data.node_count} nodes`;
